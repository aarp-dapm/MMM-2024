...
df= spark.sql('select * from default.icm_datadigital_affiliate_1_csv')
...
assert day_of_week == 2, "First week day is not Monday"


### Standardizing Names ###
df = df.withColumnRenamed('DMA_NAME','DmaName')\
               .withColumnRenamed('DMA_Code','DmaCode')\
               .withColumnRenamed('Impressions__Estimated_','Imps')\
...
final_model_df = reduce(DataFrame.unionAll, df_list)
...
final_model_df = final_model_df.withColumn('Channel', f.lit('Affiliate')).select(cols) ## Re-arranging Columns 
final_model_df.display()
...
## Saving Dataframe ##
table_name = "temp.ja_blend_mmm2_IcmDigital_Affiliate" 
