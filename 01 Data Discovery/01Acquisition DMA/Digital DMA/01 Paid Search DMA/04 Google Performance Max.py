...
df = read_table("temp.ja_blend_mmm2_DigitalCampaign").filter((f.col('media_type')=='Google Performance Max')).filter(f.year(f.col('week_start')).isin([2022, 2023]))
...
df = df.withColumn( 'SubChannel', f.lit('GooglePerfMax'))
...
## Saving Dataframe ##
table_name = "temp.ja_blend_mmm2_GooglePerfMax" 
