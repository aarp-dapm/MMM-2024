...
df = read_table("temp.ja_blend_mmm2_DigitalCampaign").filter((f.col('Traffic_Source')=='Yahoo')).filter(f.year(f.col('week_start')).isin([2022, 2023]))
...
df = df.withColumn( 'SubChannel', f.lit('Yahoo'))
...
## Saving Dataframe ##
table_name = "temp.ja_blend_mmm2_Yahoo" 
