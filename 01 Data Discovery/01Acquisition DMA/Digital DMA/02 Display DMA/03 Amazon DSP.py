...
df = read_table("temp.ja_blend_mmm2_DigitalDMA").filter((f.col('Traffic_Source')=='Amazon DSP')).filter(f.year(f.col('week_start')).isin([2022, 2023]))
...
df = df.withColumn( 'SubChannel', f.lit('AmazonDSP'))
...
## Saving Dataframe ##
table_name = "temp.ja_blend_mmm2_AmazonDSP" 
