...
df = read_table("temp.ja_blend_mmm2_DigitalCampaign").filter( (f.col('media_type')=='Social') & (f.col('Traffic_Source')=='Instagram') ).filter(f.year(f.col('week_start')).isin([2022, 2023]))
...
df = df.withColumn( 'SubChannel', f.lit('Instagram'))
...
## Saving Dataframe ##
table_name = "temp.ja_blend_mmm2_SocialInstagram" 
