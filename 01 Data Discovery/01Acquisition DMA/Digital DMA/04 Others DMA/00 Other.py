...
table_name = ["temp.ja_blend_mmm2_Nextdoor", "temp.ja_blend_mmm2_Outbrain"]
...
# campaign_maps = read_excel('dbfs:/blend360/sandbox/mmm/digital/campaign_groups/Social Campaigns Grouping.xlsx').drop('SubChannel')
...
## Saving Dataframe ##
table_name = "temp.ja_blend_mmm2_MemDigitalOther" 
save_df_func(df, table_name)

# COMMAND ----------

# df.groupBy('Date', 'MediaName', 'Channel', 'SubChannel').agg( f.sum(f.col('Spend')).alias('Spend'), f.sum(f.col('Imps')).alias('Imps'), f.sum(f.col('Clicks')).alias('Clicks') ).display()
