...
model_df[pd.to_datetime(model_df['Date']).dt.year==2023].groupby('DmaCode')[['DirectMail_Imps' ,'DirectMail_Spend']].sum().reset_index().display()
...
with pm.Model(coords=coords) as varying_intercept:
       ...
       ## Randomizing TV ##
       mu_re_ch = pm.Normal('mu_re_ch', mu=0.082, sigma=0.1)
       ...
       beta_dm_re = pm.Normal('beta_dm_re', mu = mu_re_ch, sigma=sigma_re_ch, dims='DmaCodes')       # beta_dm =   pm.Normal('beta_dm', mu= 0.082,sigma=0.005)            
...
plt.axhline(y=0.082, color = 'r', linestyle= '--', label = 'Prior')
