...
## Reaqding Data ##
rawdata_df=spark.sql('select * from temp.ja_blend_mmm2_MediaDf_Campaign_MediaCom_VideoOLV').toPandas()
rawdata_df.display()

# COMMAND ----------

for col in rawdata_df.columns:
  if col.startswith('Video_MediaCom_OLV_'):
    print(col)
...
rawdata_df['Video_MediaCom_OLV_NonBrand_Spend'] = rawdata_df[['Video_MediaCom_OLV_Foundation_Spend', 'Video_MediaCom_OLV_ODEI_Spend', 'Video_MediaCom_OLV_Programs_Spend', 'Video_MediaCom_OLV_TargetedCommunities_Spend']].sum(axis=1)
rawdata_df['Video_MediaCom_OLV_NonBrand_Imps'] = rawdata_df[['Video_MediaCom_OLV_Foundation_Imps', 'Video_MediaCom_OLV_ODEI_Imps', 'Video_MediaCom_OLV_Programs_Imps', 'Video_MediaCom_OLV_TargetedCommunities_Imps']].sum(axis=1)
rawdata_df['Video_MediaCom_OLV_NonBrand_Clicks'] = rawdata_df[['Video_MediaCom_OLV_Foundation_Clicks', 'Video_MediaCom_OLV_ODEI_Clicks',  'Video_MediaCom_OLV_Programs_Clicks', 'Video_MediaCom_OLV_TargetedCommunities_Clicks']].sum(axis=1)
...
'''
From Main Model : 'Video_Imps_AdStock6L4Wk30Ad_Power30'
'''

cols = ['Video_MediaCom_OLV_Brand_Imps', 'Video_MediaCom_OLV_NonBrand_Imps']

input_dict = {}
for col in cols:
  input_dict[col] = {'Adstock':{'week_cutoff':[6], 'peak_week':[4], 'decay_rate': [0.3]}, 'Saturation':{'power':[0.3]}}
...
# MAGIC %time
# MAGIC
# MAGIC grouped_df = rawdata_df.groupby('DmaCode')[['Date','Video_MediaCom_OLV_Brand_Imps', 'Video_MediaCom_OLV_NonBrand_Imps']]
# MAGIC
...
## Saving Model ##
transformed_df.to_csv('/dbfs/blend360/sandbox/mmm/model/MMM_only_transformed_df_statsmodel_Campaign_MediaCom_VideoOLV.csv')

## Reading Model ##
transformed_df = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/MMM_only_transformed_df_statsmodel_Campaign_MediaCom_VideoOLV.csv').drop('Unnamed: 0',axis=1)
...
model_df = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/MMM_transformed_updated_df_SubChannel_Video.csv').drop('Unnamed: 0',axis=1)
model_df.head()
...
##############################################################
submodel_cols = ['DmaCode', 'Date', 'Video_MediaCom_OLV_Brand_Spend', 'Video_MediaCom_OLV_NonBrand_Spend', 'Video_MediaCom_OLV_Brand_Imps', 'Video_MediaCom_OLV_NonBrand_Imps', 'Video_MediaCom_OLV_Brand_Clicks', 'Video_MediaCom_OLV_NonBrand_Clicks']
...
## saving model dataframe ##
model_df.to_csv('/dbfs/blend360/sandbox/mmm/model/MMM_transformed_updated_df_Campaign_MediaCom_VideoOLV.csv')
## Reading Model ##
model_df = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/MMM_transformed_updated_df_Campaign_MediaCom_VideoOLV.csv').drop('Unnamed: 0',axis=1)
model_df.head()
