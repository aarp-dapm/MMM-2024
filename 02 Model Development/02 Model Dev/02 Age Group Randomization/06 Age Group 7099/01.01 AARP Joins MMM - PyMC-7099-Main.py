...
with pm.Model(coords=coords) as varying_intercept:
       ...
       ### Priors ###
       mu_re = pm.Normal('mu_re', mu=0.1, sigma=0.001)
       sigma_re = pm.Exponential('sigma_re',0.005)
       ...
       ### Media Variables ###
       beta_tv =    pm.Normal('beta_tv', mu= 0.1,sigma=0.0005)           #  pm.Beta('beta_tv', alpha=1.5, beta=10) 
       beta_search =  pm.Normal('beta_search', mu= 0.085,sigma=0.005)      #  pm.Beta('beta_search', alpha=1.5, beta=10)
       beta_social =   pm.Uniform('beta_social', lower=0.0001, upper=1)    #  pm.Normal('beta_social', mu= 0.029,sigma=0.007) 
       ...
       beta_dm =   pm.Uniform('beta_dm', lower=0.0001, upper=1)             #  pm.Normal('beta_dm', mu= 0.082,sigma=0.005) 
       ...
       ### Other Flags ###
       beta_July0422_lag = pm.Normal('beta_July0422_lag', mu= 0.316,sigma=1) # pm.Beta('beta_July0422_lag', alpha=3, beta=10)
       ...
       y_like = pm.Normal('y_like', mu= y_hat, sigma=sd_y, observed=model_df['Joins_7099_norm'], dims='obs_id')
...
# Model Summary #
summary_df = az.summary(varying_intercept_trace, round_to=4).reset_index()
summary_df.display()
...
contribution = pd.concat([model_df[['DmaCode', 'Date', 'Joins', 'Joins_6069_norm']], contribution], axis=1) ## Change Target Var
...
contribution['Residuals'] = contribution['Joins_6069_norm'] - contribution['Pred']  ## Change Target Var
contribution['ratio_resid_join'] =  contribution['Residuals']/contribution['Joins_6069_norm'] ## Change Target Var
contribution['Year'] = pd.to_datetime(contribution['Date']).dt.year

## Update name ##
contribution.rename(columns={'Joins':'Joins_unnorm', 'Joins_6069_norm':'Joins'}, inplace=True) ## Change Target Var
...
agg_analysis_df.to_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_main_model_7099.csv')

## Read Agg. Analysis Dataframe ##
agg_analysis_df = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_main_model_7099.csv').drop('Unnamed: 0',axis=1)
