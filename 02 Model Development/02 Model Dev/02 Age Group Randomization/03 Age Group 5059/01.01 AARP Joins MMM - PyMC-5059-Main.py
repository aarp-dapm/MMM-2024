...
with pm.Model(coords=coords) as varying_intercept:
       ...
       y_like = pm.Normal('y_like', mu= y_hat, sigma=sd_y, observed=model_df['Joins_5059_norm'], dims='obs_id')
...
contribution = pd.concat([model_df[['DmaCode', 'Date', 'Joins', 'Joins_5059_norm']], contribution], axis=1) ## Change Target Var
...
contribution['Residuals'] = contribution['Joins_5059_norm'] - contribution['Pred']  ## Change Target Var
contribution['ratio_resid_join'] =  contribution['Residuals']/contribution['Joins_5059_norm'] ## Change Target Var
contribution['Year'] = pd.to_datetime(contribution['Date']).dt.year

## Update name ##
contribution.rename(columns={'Joins':'Joins_unnorm', 'Joins_5059_norm':'Joins'}, inplace=True) ## Change Target Var
...
agg_analysis_df.to_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_main_model_5059.csv')

## Read Agg. Analysis Dataframe ##
agg_analysis_df = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_main_model_5059.csv').drop('Unnamed: 0',axis=1)
