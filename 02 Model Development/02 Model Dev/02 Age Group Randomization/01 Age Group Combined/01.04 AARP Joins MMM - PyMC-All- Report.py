...
main_model = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_main_model_combined.csv').drop('Unnamed: 0',axis=1).query("Channel != 'Random Dma Effect'")[['Channel', 'Contrib_2022', 'Contrib_2023']]
...
submodel_leadgen = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_SubModel_Leadgen_combined.csv').drop('Unnamed: 0',axis=1).query("Channel != 'Random Dma Effect'")[['Channel', 'Contrib_2022', 'Contrib_2023']]
...
submodel_search = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_SubModel_Search_combined.csv').drop('Unnamed: 0',axis=1).query("Channel != 'Random Dma Effect'")[['Channel', 'Contrib_2022', 'Contrib_2023']]
...
assert abs(final_report_df['Contrib_2022_Prop'].sum()-1) < 0.01
assert abs(final_report_df['Contrib_2023_Prop'].sum()-1) < 0.01
...
final_report_df.to_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_contrib_report_combined.csv')
...
final_report_df = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_contrib_report_combined.csv').drop('Unnamed: 0',axis=1)
final_report_df.display()
