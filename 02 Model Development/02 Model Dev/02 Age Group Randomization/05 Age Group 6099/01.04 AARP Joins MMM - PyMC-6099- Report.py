...
main_model = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_main_model_6099.csv').drop('Unnamed: 0',axis=1).query("Channel != 'Random Dma Effect'")[['Channel', 'Contrib_2022', 'Contrib_2023']]
...
assert abs(main_model['Contrib_2022_Prop'].sum()-1) < 0.001
assert abs(main_model['Contrib_2023_Prop'].sum()-1) < 0.001
...
submodel_leadgen = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_SubModel_Leadgen_6099.csv').drop('Unnamed: 0',axis=1).query("Channel != 'Random Dma Effect'")[['Channel', 'Contrib_2022', 'Contrib_2023']]
...
assert abs(submodel_leadgen['Contrib_2022_Prop'].sum()-1) < 0.001
assert abs(submodel_leadgen['Contrib_2023_Prop'].sum()-1) < 0.001
...
submodel_search = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_SubModel_Search_6099.csv').drop('Unnamed: 0',axis=1).query("Channel != 'Random Dma Effect'")[['Channel', 'Contrib_2022', 'Contrib_2023']]
...
final_report_df.to_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_contrib_report_6099.csv')

## Read Agg. Analysis Dataframe ##
final_report_df = pd.read_csv('/dbfs/blend360/sandbox/mmm/model/agg_analysis_df_contrib_report_6099.csv').drop('Unnamed: 0',axis=1)
final_report_df.display()
