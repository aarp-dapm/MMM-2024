...
## Reading Model ##
model_path = '/dbfs/blend360/sandbox/mmm/model/MMM_StatsModel_Apr2_PE_Vo5.pkl'
...
reg = model_df[['DmaCode', 'Date','PE_Total_norm', 'PE_Total']]

## Renaming Variables ##
reg = reg.rename(columns = { 'PE_Total':'PE_Total_unnorm', 'PE_Total_norm':'PE_Total'}) 
...
pred_df_date = pred_df.groupby('Date').agg({'PE_Total':'sum', 'Pred':'sum'}).reset_index()

## Plots ##
plt.figure(figsize=(25,6))
plt.plot( pred_df_date.PE_Total, label='Actual')
...
residuals = pred_df_date.PE_Total - pred_df_date.Pred
...
mape = mean_absolute_percentage_error(pred_df_date.PE_Total, pred_df_date.Pred)
r_squared = r2_score(pred_df_date.PE_Total, pred_df_date.Pred)
...
contribution['ratio_resid_join'] = contribution['Residual']/contribution['PE_Total']
